...
export async function generateMetadata({ params }: NichePageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `/instagram-engagement-rate-calculator/${data.slug}`,
    },
    alternates: {
      canonical: `/instagram-engagement-rate-calculator/${data.slug}`,
    },
  };
}
  ...
  const howItWorks = (
    <>
      ...
      <ul className="mt-2 list-disc space-y-1 pl-5">
        ...
        <li>
          <Link
            href={`/instagram-sponsorship-rate-calculator/${data.slug}`}
            className="font-medium text-primary hover:underline"
          >
            Instagram {data.name} Sponsorship Rates
          </Link>{' '}
    ...
    <>
      ...
      <CalculatorLayout
        ...
        breadcrumbs={[
          ...
          { name: 'Engagement Rate', path: '/instagram-engagement-rate-calculator' },
          { name: data.name, path: `/instagram-engagement-rate-calculator/${data.slug}` },
        ]}
      >
        <Suspense>
          <EngagementCalculator
            config={{ ...INSTAGRAM_CONFIG, defaultIndustryId: data.industryId }}
          />
        </Suspense>
