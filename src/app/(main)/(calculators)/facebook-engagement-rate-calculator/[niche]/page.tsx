...
export async function generateMetadata({ params }: NichePageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `/facebook-engagement-rate-calculator/${data.slug}`,
    },
    alternates: {
      canonical: `/facebook-engagement-rate-calculator/${data.slug}`,
    },
  };
}
  ...
  const howItWorks = (
    <>
      ...
      <ul className="mt-2 list-disc space-y-1 pl-5">
        ...
        <li>
          <Link
            href={`/facebook-sponsorship-rate-calculator/${data.slug}`}
            className="font-medium text-primary hover:underline"
          >
            Facebook {data.name} Sponsorship Rates
          </Link>{' '}
    ...
    <>
      ...
      <CalculatorLayout
        ...
        breadcrumbs={[
          ...
          { name: 'Engagement Rate', path: '/facebook-engagement-rate-calculator' },
          { name: data.name, path: `/facebook-engagement-rate-calculator/${data.slug}` },
        ]}
      >
        <Suspense fallback={<CalculatorSkeleton />}>
          <EngagementCalculator
            config={{ ...FACEBOOK_CONFIG, defaultIndustryId: data.industryId }}
          />
        </Suspense>
