...
import { useTikTokSponsorshipState } from './useTikTokSponsorshipState';
import TikTokSponsorshipShareButtons from './ShareButtons';
...
export default function TikTokSponsorshipCalculator() {
  ...
  const { state, dispatch } = useTikTokSponsorshipState();
  ...
  const tips = useMemo(
    () => getNegotiationTips(result.tier, state.engagementRate, 'tiktok'),
      ...
      <Card>
        <div className="space-y-6">
          <ContentTypeSelector
            ...
            onChange={(v) =>
              dispatch({ type: 'SET_CONTENT_TYPE', payload: v as TikTokContentType })
            }
          />
